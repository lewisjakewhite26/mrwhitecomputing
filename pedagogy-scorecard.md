# Pedagogy scorecard: before vs after, all 18 lessons

All 18 lessons are now built. This replaces the earlier "current vs
projected" version with an honest before/after: what the lesson scored
before this work started, and what it actually scores now that the build
is done, not what was originally planned. Several builds turned out
differently from the plan in `enrichment-research.md`, mostly smaller and
...
| Band | What it means |
|---|---|
| 80–100 | A checked, hands-on activity is embedded in the deck itself, reveal-card load is low, pupils get real feedback on whether they're right |
| 60–79 | A checked activity exists, but it's either a late addition rather than core to the lesson, or a strong external resource with its own built-in check |
| 40–59 | Something exists for pupils to do: a worksheet, a demo, a printed sort. It isn't checked or paced inside the deck |
| 20–39 | Mostly or entirely tap-to-reveal cards. The real work is a printed sheet the deck never touches |
...
| # | Lesson | Before | What actually got built | After |
|---|---|---|---|---|
| 1 | Systems | **80** | Untouched this session; already had two working checked mechanics and wasn't in the build queue | **80** |
| 2 | Computer systems and us | **20** | The worst ratio in the whole curriculum (21 reveal cards, zero checked activity) now has a new 3-bin sort (Person / Both together / Computer) across four fresh systems: self-checkout, warehouse robot, smart thermostat, traffic lights | **76** |
| 3 | Searching the web | **36** | The Rugby/Sale/Boot examples became a checked sequence activity per place name: first search, why it failed, better search | **79** |
| 4 | Selecting search results | **42** | A checked "search the index, not the book" mini-quiz was added right after the index concept is introduced, smaller in scope than the full Barefoot crawler-building activity from the research, but a genuine checked practice of the exact misconception this lesson targets | **74** |
| 5 | How search results are ranked | **40** | The static point-value reveal cards became a live interactive scorer: groups count real occurrences on their own paper webpage and tap + for each one, with the total tallying on screen. The separate Barefoot Ranking activity wasn't added on top | **78** |
| 6 | How are searches influenced | **22** | The second-worst ratio in the curriculum (25 reveal cards across 8 slides): the three "What influences X" slides became one checked 3-bin sort (11 factors), the Priya/Marcus/Sophie cards became a checked sequence activity, and a TED-Ed video was added as a bridge to the next unit | **80** |

**Unit average: 40.0 before, 77.8 after.**
...
work. The after-scores land a little under the original projections in
`enrichment-research.md` in a handful of lessons (Y3/4 L3, L4, L6; Y5/6 L4,
L5), because the actual builds are homegrown checked activities rather than
the full external resources those projections assumed. That's a fair
trade: every one of those lessons still gained a real, working, checked
activity where it had nothing before. None of the builds fall short of
"something checked now exists"; they just don't all match the single best
possible resource that was scouted for them.
