...
- **iPad photo scavenger hunt (Lewis's idea)**: instead of only sorting cards or pointing, pupils walk the classroom in pairs and photograph each piece of technology they find, using the iPad's own Camera app. Back together, the class looks through a few of the photos and sorts them into the same Technology/Not Technology categories, out loud. It puts real technology in pupils' hands as part of learning to identify it; the activity is itself an example of its own subject matter. No custom build needed, since the Camera app already does the job well. **Score: 68/100** — no published resource to point to, but a concrete, zero-cost activity using kit the school already has, and a clear step up from the generic "walk and point" idea it replaces. Docked slightly from the original pitch because the in-app photo sort didn't make it into the build; the sorting now happens as class discussion, not a checked slide.
...
- **Quick Click (built for this project, `quick-click.html`)**: a reaction-test click game, built after Lewis pointed out the school has no mice for pupils to use directly. A dot spawns at random and grows; click it, quick and central scores best, but it never disappears, so nobody "loses." Works as a real "have a go" after the demo instead of only watching code.org, with a built-in score for feedback. Designed to run as a 1:1 turn at the teacher's PC while the rest of the class continues another task, which solves the no-mice problem the earlier Mouse House/Dragon Drop pick didn't. **Score: 74/100**: purpose-built for this exact constraint, not generic mouse-training, and it reuses no third-party site that could go offline.
