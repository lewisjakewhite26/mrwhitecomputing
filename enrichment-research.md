...
- **Extend the existing sort activity** with a "walk and point" round — pupils physically point to school technology (whiteboard, tablet, camera) rather than only sorting cards. A sound technique, not tied to a specific published resource. **Score: 45/100** — cheap and embodied, but generic and unverified against a named resource.
...
- Strengthen the password activity with an unplugged "spot the weak password" checked sort (pupils sort pre-written example passwords into strong/weak, justifying why) — a sound generic idea, not a discovered resource. **Score: 40/100**.
